...
<input type="checkbox" name="${parameters.name?html}" value="${itemKeyStr?html}"
    ...
    <#include "/${parameters.templateDir}/simple/common-attributes.ftl" />
        />
<#-- change the label tag has a style of red bold text. -->
<label for="${parameters.name?html}-${itemCount}" class="checkboxLabel" style="color:red; font-weight:bold;">${itemValue?html}</label>
<#-- have a break tag after each label tag so that each checkbox and its label will be on their own line. -->
<br>
</@s.iterator>
    <#else>
    &nbsp;
